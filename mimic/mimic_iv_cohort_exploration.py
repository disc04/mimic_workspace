...
from utils import cohort_analysis_utils
from configuration import severity_scores_dictionary, drug_categories_dictionary
...
def analyze_multiple_admissions(hosp_tables: dict,
    ...
    cohort_analysis_utils.run_demographics_analysis(admissions=results_dict["admission"],
                                                    icu_stays=results_dict["vitals"],
                                                    diagnoses=results_dict["diagnoses"],
                                                    results_path=results_path)
    cohort_plotting_utils.plot_services_analysis(hosp_tables,
                                                 join(results_path, "Hospital_services_analysis.png"))

    cohort_analysis_utils.run_admission_readmission_analysis(results_dict["admission"],
                                                             results_path)

    cohort_analysis_utils.run_severity_mortality_analysis(results_dict["admission"],
    ...
    cohort_analysis_utils.run_lab_tests_analysis(results_dict["labs"],
                                                 results_path)
    cohort_analysis_utils.run_prescriptions_analysis(results_dict["prescription_medications"],
                                                     drug_categories_dictionary,
                                                     results_path)
    cohort_analysis_utils.run_procedures_analysis(results_dict["procedures"],
